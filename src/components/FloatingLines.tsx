...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableMiddle) {
    for (int i = 0; i < middleLineCount; ++i) {
      ...
      vec2 ruv = baseUv * rotate(angle);
      float w = wave(ruv + vec2(middleLineDistance * fi + middleWavePosition.x, middleWavePosition.y), 2.0 + 0.15 * fi, baseUv, mouseUv, interactive) * 0.4;
      col += vec4(lineCol * w, w);
    }
  }
  ...
  fragColor = vec4(col.rgb, min(col.a, 1.0));
}
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // Use alpha: true so the background is completely transparent
    ...
    const material = new ShaderMaterial({
      ...
      vertexShader,
      fragmentShader,
      transparent: true // Enable native transparency support in three.js
    });
    ...
    const handlePointerMove = (event: PointerEvent) => {
      // By binding to the window, the mouse position works globally even when pointer-events-none hides this canvas
